...
public class AjouterStagiaire extends JFrame {
	
	private static final long serialVersionUID = 1L;
	JPanel pan;
	JLabel nom, prenom, datenaissance, sexe, adresse, cp, ville, pays, mail, mdp;
	JTextField TFnom, TFprenom, TFdatenaissance, TFadresse, TFcp, TFville, TFpays, TFmail, TFmdp;
	JComboBox<String> CBsexe;
	JButton retour, ajout;
	String nomS="", prenomS="",datenaissanceS="",sexeS="",adresseS="",cpS="",villeS="",paysS="",mailS="",pseudo="",mdpS="";
	
	public AjouterStagiaire() {
		setTitle("Ajouter un Stagiaire");
		setIconImage(Toolkit.getDefaultToolkit().getImage(AjouterStagiaire.class.getResource("/image/logo MFC.png")));
		...
		mdp = new JLabel("MDP :");
		mdp.setFont(new Font("Times New Roman", Font.PLAIN, 15));
		mdp.setBounds(590, 238, 76, 26);
		pan.add(mdp);
		
		TFmdp = new JTextField();
		TFmdp.setBounds(650, 238, 244, 20);
		pan.add(TFmdp);
		TFmdp.setColumns(11);
		...
		ajout.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				mdpS= TFmdp.getText();
				new TraitementStagiaire().ajoutStagiaire(prenomS, nomS, datenaissanceS, sexeS, adresseS, villeS, cpS,
		 					 paysS, mailS, pseudo, mdpS);
				Stagiaire session = new Stagiaire();
				session.setVisible(true);
				dispose();
			}
